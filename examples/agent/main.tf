...
module "apollo" {
  ...
  # Required for new deployments. Generate the External ID in Monte Carlo
  # (Settings → Deployments → Add deployment) and replace the placeholder
  # below before running `terraform apply`. Leave blank only for existing
  # deployments that already use the random ID generated by this module.
  external_id = "REPLACE_WITH_MONTE_CARLO_EXTERNAL_ID"
}
